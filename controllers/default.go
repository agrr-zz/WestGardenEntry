...
//展示活动列表
func (c *MainController) ShowEvent() {
	o := orm.NewOrm()
	// id, _ := c.GetInt("select")

	var evlst []models.EventList
	_, err := o.QueryTable("EventList").All(&evlst)
	if err != nil {
		beego.Info("no event")
		return
	}

	// var mlst []models.MemberList
	// o.QueryTable("MemberList").All(&mlst)

	c.Data["evlst"] = evlst
	// c.Data["mlst"] = mlst
	c.TplName = "eventList.html"
}

//展示活动参加人员名单
func (c *MainController) ShowEventMember() {
	o := orm.NewOrm()
	// id, _ := c.GetInt("select")
	...
	c.Data["evlst"] = evlst
	c.Data["mlst"] = mlst
	c.TplName = "eventMember.html"
}

//新增人员处理
func (c *MainController) AddMember() {
	//1.拿到数据
	uName := c.GetString("uName")
	uPhone := c.GetString("uPhone")
	// id, err := c.GetInt("select")
	// if err != nil {
	// 	beego.Info("获取类型错误")
	// 	return
	...
	//2.判断数据是否合法
	if uName == "" || uPhone == "" {
		beego.Info("please check your commit")
		return
	}
	//3.插入数据
	o := orm.NewOrm()
	mem := models.MemberList{}
	mem.MemberName = uName
	mem.Phone = uPhone

	_, err := o.Insert(&mem)
	if err != nil {
		beego.Info("insert wrong")
		return
	}

	//4.返回文章界面
	c.Redirect("/eventMember", 302)
}
...
func (c *MainController) AddEvent() {
	...
	eStartDate := c.GetString("eStartDate")
	eStartTime := c.GetString("eStartTime")
	eContinue := c.GetString("eContinue")
	// id, err := c.GetInt("select")
	// if err != nil {
	// 	beego.Info("获取类型错误")
	// 	return
	...
	//2.判断数据是否合法
	if eName == "" || eStartTime == "" || eContinue == "" || eStartDate == "" {
		beego.Info("please check your commit")
		return
	}
	//3.插入数据
	o := orm.NewOrm()
	elst := models.EventList{}
	elst.Eventname = eName
	elst.EventStartDate = eStartDate
	elst.EventStartTime = eStartTime
	elst.EventContinue = eContinue
